...
class MakeDoScreen extends StatefulWidget {
  final Iterable<int> makeDoIds;

  const MakeDoScreen(this.makeDoIds, {super.key,});
...
class _MakeDoScreen extends State<MakeDoScreen> {
  Widget _mainContent(MakeDoLoadedState state) {
    final Iterable<MakeDoUiModel> modelList = state.model;

    final widgetList = modelList.map((model) {
  ...
  Widget build(BuildContext context) {
    return AppScaffold(
      ...
      body: BlocProvider(
          create: (_) => MakeDoCubit(widget.makeDoIds),
